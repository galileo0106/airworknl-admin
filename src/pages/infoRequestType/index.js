...
const InfoRequestTypePage = () => {
      ...
      renderCell: (params) => {
        return (
          <Stack direction="row" spacing={1}>
            ...
            <IconButton
              onClick={(e) => {
                navigate(`/info_request_type/${params.id}/edit`);
              }}
            >
              <EditIcon />
            </IconButton>
  ...
  const handleDelete = async () => {
    try {
      ...
      await apis.deleteInfoRequestTypes(data.join());
      init();
      setOpenDialog({ ...openDialog, delete: false });
    } catch (error) {
      console.log(error);
  ...
  return (
    <Box>
      <Dialog open={openDialog.delete} onClose={handleCloseDeleteDialog}>
        <DialogTitle>Delete info request types</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Are you sure to delete selected info request types?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Stack
          ...
          <Stack direction="column" spacing={2}>
            <Stack direction="row" spacing={2} justifyContent="space-between">
              <Stack direction="row" spacing={1} alignItems="center">
                ...
                <Typography variant="h6">
                  Manage information request types
                </Typography>
              </Stack>
              <Stack direction="row" spacing={1} alignItems="center">
                <Button
            ...
            <Box height={600}>
              <DataGrid
                ...
                localeText={{ noRowsLabel: "No info_request types" }}
                rows={rows}
                columns={columns}
...
export default InfoRequestTypePage;
