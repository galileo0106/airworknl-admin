...
const SimulatorPage = () => {
      ...
      renderCell: (params) => {
        return (
          <Stack direction="row" spacing={1}>
            ...
            <IconButton
              onClick={(e) => {
                navigate(`/simulator/${params.id}/edit`);
              }}
            >
              <EditIcon />
            </IconButton>
  ...
  const handleDelete = async () => {
    try {
      ...
      await apis.deleteSimulators(data.join());
      init();
      setOpenDialog({ ...openDialog, delete: false });
    } catch (error) {
      console.log(error);
  ...
  return (
    <Box>
      <Dialog open={openDialog.delete} onClose={handleCloseDeleteDialog}>
        <DialogTitle>Delete simulators</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Are you sure to delete selected simulators?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Stack
          ...
          <Stack direction="column" spacing={2}>
            <Stack direction="row" spacing={2} justifyContent="space-between">
              <Stack direction="row" spacing={1} alignItems="center">
                ...
                <Typography variant="h6">Manage simulators</Typography>
              </Stack>
              <Stack direction="row" spacing={1} alignItems="center">
                <Button
                  variant="contained"
