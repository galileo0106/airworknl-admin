...
const InstituteSimulatorPage = () => {
  ...
  const handleDelete = async () => {
    try {
      ...
      await apis.deleteInstituteSimulators(data.join());
      init();
  ...
  const handleView = (row) => async (event) => {
    try {
      setSelectedRow(row);
  ...
  const init = async () => {
    try {
      const {
        data: { instituteSimulators },
      } = await apis.getInstituteSimulators(`?institutes=${institute_id}`);
      setRows(instituteSimulators);
    } catch (error) {}
  };
  ...
  return (
    <Box>
      <Dialog open={openDialog.delete} onClose={handleCloseDeleteDialog}>
        <DialogTitle>Remove simulators</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Are you sure to remove selected simulators from this institute?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Stack
      ...
      <Stack direction="column" spacing={2}>
        <Stack direction="row" spacing={2} justifyContent="space-between">
          <Box gap={1} display="flex">
            ...
            <Button
              variant="contained"
              onClick={() =>
                navigate(`/institute/${institute_id}/simulator/create`)
              }
              startIcon={<AddIcon />}
            >
              Add simulator
        ...
        <Box height={500}>
          <DataGrid
            sx={{ height: "100%" }}
            localeText={{ noRowsLabel: "No simulators" }}
            rows={rows}
            columns={columns}
...
export default InstituteSimulatorPage;
